...
import { getByesData } from 'competitionFactory/drawEngine/governors/positionGovernor/positionByes';
import { getQualifiersData } from 'competitionFactory/drawEngine/governors/positionGovernor/positionQualifiers';
import {
  structureAssignedDrawPositions,
  structureActiveDrawPositions
} from 'competitionFactory/drawEngine/getters/positionsGetter';

import { WILDCARD, DIRECT_ACCEPTANCE } from 'competitionFactory/constants/drawDefinitionConstants';

/*
  return an array of all possible validActions for a given drawPosition within a structure
*/
export function positionActions({drawDefinition, participantId, structureId, drawPosition}) {
  const { structure } = findStructure({ drawDefinition, structureId });

  let validActions = [];

  if (!structure) {
    console.log('no structure found', {drawDefinition, structureId, drawPosition, participantId});
    return { validActions };
  }

  const { assignedPositions } = structureAssignedDrawPositions({ structure });
  const positionAssignment = assignedPositions.reduce((positionAssignment, assignment) => {
    return assignment.drawPosition === drawPosition ? assignment : positionAssignment;
  }, undefined);
  ...
  if (!positionAssignment) {
    ...
    if (unfilledPositions.includes(drawPosition)) {
      const validToAssign = unplacedSeedAssignments
        .filter(seedAssignment => unplacedSeedParticipantIds.includes(seedAssignment.participantId))
        .map(valid => Object.assign(valid, { drawId, structureId, drawPosition }));

      validToAssign.sort(validAssignmentsSort);
      console.log({ validToAssign, unfilledPositions, unplacedSeedAssignments, structure})
      ...
      const assignedParticipantIds = assignedPositions.map(assignment => assignment.participantId);
      ...
      const { byesCount, placedByes, validByePositions } = getByesData({drawDefinition, structure});
      ...
      if (validPositionForBye && unassignedByes) {
        validToAssign.push({ bye: true, unassignedByes, drawId, structureId, drawPosition });
      }
    
      // discover how many qualifiers are unplaced
      const { unplacedQualifiersCount } = getQualifiersData({drawDefinition, structure});
    ...
    const { activeDrawPositions, byeDrawPositions } = structureActiveDrawPositions({drawDefinition, structureId});
    if (!activeDrawPositions.includes(drawPosition)) {
      validActions.push({ type: 'REMOVE', payload: { drawId, structureId, drawPosition } });
    }
...
export function getNextUnfilledDrawPositions({drawDefinition, structureId}) {
  ...
  const { unfilledPositions } = getNextSeedBlock({drawDefinition, structureId, randomize: true});
