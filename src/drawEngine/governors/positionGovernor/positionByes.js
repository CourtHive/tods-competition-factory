...
export function assignDrawPositionBye({drawDefinition, structureId, drawPosition}) {
  ...
  const { activeDrawPositions } = structureActiveDrawPositions({drawDefinition, structureId});
  ...
  if (drawPositionFilled(positionState)) return { error: 'Draw position is occupied' };
  if (drawPositionIsActive) return { error: 'Cannot place bye in active drawPosition' };
  ...
  matchUps.forEach(matchUp => {
    if (matchUp.drawPositions.includes(drawPosition)) {
      ...
      const pairedDrawPosition = matchUp.drawPositions.reduce((pairedDrawPosition, currentDrawPosition) => {
        return currentDrawPosition !== drawPosition ? currentDrawPosition : pairedDrawPosition;
      }, undefined);
      ...
      if (loserMatchUp) {
        ...
        const loserMatchUpDrawPositionsCount = loserMatchUp.drawPositions.filter(f=>f).length;
        if (loserMatchUpDrawPositionsCount !== 2) return { error: 'Missing drawPositions in loserMatchUp' };
...
export function positionByes({drawDefinition, structure, structureId, blockOrdered=false}) {
  ...
  const {
    ...
    placedByes,
    relevantMatchUps
  } = getByesData({drawDefinition, structure});
  ...
  const seedOrderByePositions = blockOrdered ? blockSeedOrderByePositions : strictSeedOrderByePositions;
 
  const { unseededByePositions } = getUnseededByePositions({structure, isFeedIn});
  ...
  let byePositions = [].concat(...seedOrderByePositions, ...unseededByePositions);
  ...
  for (const drawPosition of byeDrawPositions) {
    const result = drawEngine.assignDrawPositionBye({structureId, drawPosition});
...
function getUnseededByePositions({structure, isFeedIn}) {
  ...
  const matchUpFilters = { isCollectionMatchUp: false };
  const { matchUps, roundMatchUps } = getAllStructureMatchUps({structure, matchUpFilters});
  ...
  const filledRelevantDrawPositions = filledDrawPositions
  ...
  const getHalves = chunk => {
    ...
    const [greaterHalf, lesserHalf] = (!shortLength || unequalHalves) ?
      [halves[longIndex], halves[1-longIndex]] : [shuffledHalves[0], shuffledHalves[1]];
  ...
  const { validSeedBlocks } = getValidSeedBlocks({structure, allPositions: true});
  const validBlockDrawPositions = validSeedBlocks
    .map(block => block.drawPositions
  ...
  if (isFeedIn) {
    ...
    const blockDrawPositions = Object.keys(seedBlocks)
      ...
      .map(key => {
        const seedDrawPositions = seedBlocks[key].map(d => +d[0] + baseDrawSize * d[1]);
        return seedDrawPositions.map(drawPosition => drawPosition + drawPositionOffset);
      });
  ...
  const matchUpPairedDrawPositions = relevantMatchUps.map(matchUp => matchUp.drawPositions)
  ...
  const findDrawPositionPair = drawPosition => {
    return matchUpPairedDrawPositions.reduce((pair, candidate) => {
      return candidate.includes(drawPosition) ? candidate.reduce((p,c) => c !== drawPosition ? c : p, undefined) : pair;
    }, undefined);
  };
...
function getSeedOrderByePositions({structure, relevantMatchUps}) {
  const { validSeedBlocks, isFeedIn, isContainer } = getValidSeedBlocks({structure});
  ...
  const blockSortedRandomDrawPositions = []
    .concat(...validSeedBlocks.map(seedBlock => shuffleArray(seedBlock.drawPositions)))
    .filter(drawPosition => relevantDrawPositions.includes(drawPosition));
  ...
  const orderedSortedFirstRoundSeededDrawPositions = valueOrderedBlockSortedPositionedSeeds
    .map(positionedSeed => positionedSeed.drawPosition);
  ...
  return { strictSeedOrderByePositions, blockSeedOrderByePositions, isFeedIn, isContainer };
}
...
function getOrderedByePositions({orderedSeedDrawPositions, relevantMatchUps}) {
  ...
  const seedOrderSortedDrawPositionPairs = orderedSeedDrawPositions.map(drawPosition => {
    return consideredDrawPositionPairs.reduce((drawPositionPair, drawPositions) => {
      if (drawPositionPair) return drawPositionPair; // take the first occurrence
      return drawPositions.includes(drawPosition) ? drawPositions : drawPositionPair;
    }, undefined);
  }).filter(f=>f);
  const orderedByePositions = seedOrderSortedDrawPositionPairs.map(drawPositions => {
    return drawPositions.reduce((byePosition, drawPosition) => {
      return orderedSeedDrawPositions.includes(drawPosition) ? byePosition : drawPosition;
    }, undefined);
  }).filter(f=>f);
...
export function getByesData({drawDefinition, structure}) {
  ...
  const validByePositions = relevantMatchUps.map(matchUp => matchUp.drawPositions)
    .filter(drawPositions => {
      return drawPositions && drawPositions
        .reduce((noBye, drawPosition) => !placedByePositions.includes(drawPosition) && noBye, true);
  ...
  return { placedByes, byesCount, relevantMatchUps, placedByePositions, roundMatchUps, validByePositions };
}
