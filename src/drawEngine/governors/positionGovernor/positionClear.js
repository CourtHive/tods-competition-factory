...
export function clearDrawPosition({drawDefinition, structureId, participantId, drawPosition}) {
  ...
  const { positionAssignments } = structureAssignedDrawPositions({drawDefinition, structure});
  ...
  if (drawPositionIsActive) return { error: 'Cannot clear an active drawPosition' };
  ...
  const { matchUps } = getAllStructureMatchUps({structure, matchUpFilters, inContext: true});
  matchUps.forEach(matchUp => {
    if (matchUp.drawPositions.includes(drawPosition)) {
      const isByeMatchUp = matchUp.drawPositions.reduce((isByeMatchUp, drawPosition) => {
  ...
  function removeByeAndCleanUp({matchUp, drawPosition}) {
    ...
    if (loserMatchUp && loserMatchUp.matchUpStatus === BYE) {
      ...
      clearDrawPosition({drawDefinition, structureId, drawPosition: targetDrawPosition});
    }

    if (winnerMatchUp) {
      if (winnerTargetLink && matchUp.structureId !== winnerTargetLink.target.structureId) {
        ...
        console.log('%c ALERT: remove winner participantId from target structure positionAssignments', 'color: red')
      }
