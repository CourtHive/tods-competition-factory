...
export function getSwapOptions({
  ...
  return avoidanceConflicts
    .map(conflict => {
      ...
      const swapOptions = moveableParticipants.map(moveableParticipant => {
        const possibleDrawPositions = potentialDrawPositions.filter(
          position => !drawPositions.includes(position)
        );

        const possiblePositionsNoConflict = possibleDrawPositions.filter(
        ...
        return {
          ...
          possibleDrawPositions: possiblePositionsNoConflict,
        };
      });
