...
export function assignMatchUpDrawPosition({drawDefinition, matchUpId, drawPosition}) {
  ...
  return positionAssigned ? SUCCESS : { error: 'No unfilled positions to assign' };
}

export function removeMatchUpDrawPosition({drawDefinition, matchUpId, drawPosition}) {
  const { matchUp } = findMatchUp({drawDefinition, matchUpId});
  ...
  return positionRemoved ? SUCCESS : { error: 'Not removed: drawPosition not found' };
}