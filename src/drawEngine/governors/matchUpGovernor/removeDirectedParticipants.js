...
export function removeDirectedParticipants(props) {
  ...
  const loserParticipantId = positionAssignments.reduce((participantId, assignment) => {
    return assignment.drawPosition === loserDrawPosition ? assignment.participantId : participantId;
  }, undefined);
  const winnerParticipantId = positionAssignments.reduce((participantId, assignment) => {
  ...
  if (winnerMatchUp) {
    const { error } = removeDirectedWinner({drawDefinition, winnerTargetLink, winnerParticipantId, winningDrawPosition, winnerMatchUp});
    if (error) errors.push(error);
  }
  if (loserMatchUp) {
    const { error } = removeDirectedLoser({drawDefinition, loserTargetLink, loserParticipantId});
    if (error) errors.push(error);
  }
...
function removeDirectedWinner({drawDefinition, winnerTargetLink, winnerParticipantId, winningDrawPosition, winnerMatchUp}) {
  let error;
  ...
  if (winnerTargetLink) {
    ...
    const { positionAssignments } = structureAssignedDrawPositions({structure});
    const winnerDrawPosition = positionAssignments.reduce((winnerDrawPosition, assignment) => {
      return assignment.participantId === winnerParticipantId ? assignment.drawPosition : winnerDrawPosition;
    }, undefined);

    const { matchUps } = getAllStructureMatchUps({structure});
    const allDrawPositionInstances = matchUps.map(matchUp => matchUp.drawPositions).flat(Infinity).filter(f=>f);
    ...
    const { matchUp } = findMatchUp({drawDefinition, matchUpId: winnerMatchUp.matchUpId});
...
function removeDirectedLoser({drawDefinition, loserTargetLink, loserParticipantId}) {
  let error;
