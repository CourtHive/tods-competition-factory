...
import { BYE, COMPLETED, INCOMPLETE } from '../../../constants/matchUpStatusConstants';
...
export function noDownstreamDependencies(props) {
  ...
  if (winningSide) {
    ...
    const { errors: incompleteScoreErrors } = attemptToSetIncompleteScore(props);
    if (incompleteScoreErrors) errors = errors.concat(incompleteScoreErrors);
  } else if (!winningSide && matchUp.winningSide && !score) {
    const { errors: participantDirectionErrors } = removeDirectedParticipants(props);
...
function attemptToSetMatchUpStatus(props) {
  ...
  if (matchUp.winningSide) {
    ...
    const matchUpIncludesBye = matchUp.drawPositions.reduce((includesBye, position) => {
