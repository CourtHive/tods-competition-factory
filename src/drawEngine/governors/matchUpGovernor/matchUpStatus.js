...
export function setMatchUpStatus(props) {
  ...
  if (!matchUp) {
    ...
    const { targetMatchUps: { loserMatchUp, winnerMatchUp } } = targetData;
    ...
    const winnerMatchUpHasWinningSide = winnerMatchUp && winnerMatchUp.winningSide;
    const activeDownstream = loserMatchUpHasWinningSide || winnerMatchUpHasWinningSide;
    ...
    if (!activeDownstream) {
      ...
      const { errors: winnerWithDependencyErrors } = winningSideWithDownstreamDependencies(props);
      if (winnerWithDependencyErrors) errors = errors.concat(winnerWithDependencyErrors);
...
function attemptStatusChange(props) {
  ...
  if (isDirectingMatchUpStatus({matchUpStatus})) {
    if (matchUp.winningSide && matchUpStatus !== BYE) {
      ...
      errors.push({ error: 'matchUp with winningSide cannot have matchUpStatus: BYE' });
      // TESTED
    }
  } else if (isNonDirectingMatchUpStatus({matchUpStatus})) {
    errors.push({ error: 'matchUp has winner; cannot apply non-directing matchUpStatus' });
    // TESTED
  } 
  return errors.length ? { errors } : SUCCESS;
}
...
function winningSideWithDownstreamDependencies(props) {
  ...
  if (winningSide === matchUp.winningSide) {
    if (matchUpStatus) {
      if (isDirectingMatchUpStatus({matchUpStatus}) && matchUpStatus !== BYE) {
        applyMatchUpValues(props);
        // TESTED
      } else {
        // matchUpStatus can't be changed to something non-directing
        errors.push({ error: 'Cannot change matchUpStatus to nonDirecting outcome with winningSide' });
        // TESTED
      }
    } else {
