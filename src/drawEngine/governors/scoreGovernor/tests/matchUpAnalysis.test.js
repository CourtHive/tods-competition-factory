...
test('can properly analyze completed standard format matchUp', () => {
  ...
  analysis = analyzeMatchUp({matchUp, setNumber: 3, sideNumber: 1, isTiebreakValue: true});
  expect(analysis.isActiveSet).toEqual(false);
  expect(analysis.hasExistingValue).toEqual(true);
  expect(analysis.existingValue).toEqual(7);
  analysis = analyzeMatchUp({matchUp, setNumber: 3, sideNumber: 2, isTiebreakValue: true});
  expect(analysis.isActiveSet).toEqual(false);
  expect(analysis.hasExistingValue).toEqual(true);
...
test('can properly analyze completed standard doubles matchUp', () => {
  const matchUp = {
    sets: [
      { setNumber: 1, side1Score: 6, side2Score: 3, winningSide: 1 },
      { setNumber: 2, side1Score: 3, side2Score: 6, winningSide: 2 },
      { setNumber: 3, side1TiebreakScore: 10, side2TiebreakScore: 3, winningSide: 1 },
    ],
    winningSide: 1,
...
test('can properly analyze completed Best of 3 tiebreak sets', () => {
  ...
  const matchUp = {
    sets: [
      { setNumber: 1, side1TiebreakScore: 10, side2TiebreakScore: 3, winningSide: 1 },
      { setNumber: 2, side1TiebreakScore: 7, side2TiebreakScore: 10, winningSide: 2 },
      { setNumber: 3, side1TiebreakScore: 10, side2TiebreakScore: 3, winningSide: 1 },
    ],
    winningSide: 1,
