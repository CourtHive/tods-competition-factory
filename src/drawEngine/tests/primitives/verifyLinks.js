...
export function verifyLinks({
  ...
  linksProfiles.forEach(linksProfile => {
    let { sourceStructureId, targetStructureId, linkProfiles } = { ...linksProfile };
    linkProfiles = linkProfiles.sort((a, b) => a.linkedRounds[0] - b.linkedRounds[0]);

    const filteredLinks = links.filter(link => {
      return link.source.structureId === sourceStructureId && link.target.structureId === targetStructureId;
    ...
    linkProfiles.forEach((linkProfile, i) => {
      ...
      const linkedRounds = [filteredLink.source.roundNumber, filteredLink.target.roundNumber];
