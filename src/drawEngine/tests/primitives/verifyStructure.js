...
export function verifyStructure({
  ...
  if (expectedQualifierAssignments !== undefined) {
    expect(qualifierAssignedDrawPositions.length).toEqual(expectedQualifierAssignments);
  }
  ...
  const { matchUps, roundMatchUps } = getAllStructureMatchUps({structure, inContext: true});
  ...
  if (expectedRoundMatchUpsCounts) {
    expectedRoundMatchUpsCounts.forEach((expectation, index) => {
      ...
      const roundMatchUpsCount = (roundMatchUps[roundNumber] && roundMatchUps[roundNumber].length) || 0;
  ...
  const getPairedDrawPosition = drawPosition => pairedDrawPositions.reduce((pairedDrawPosition, candidate) => {
    return candidate.includes(drawPosition) ? candidate.reduce((p, c) => c !== drawPosition ? c : p, undefined) : pairedDrawPosition;
  }, undefined);

  const seedPairedDrawPositions = seedAssignedDrawPositions.map(getPairedDrawPosition).filter(f=>f);
  const seedPairedDrawPositionsWithBye = seedPairedDrawPositions.filter(drawPosition => byeAssignedDrawPositions.includes(drawPosition));
  ...
  const seedDrawPositionsWithBye = seedPairedDrawPositionsWithBye.map(getPairedDrawPosition);
  const seedValuesOfSeedsWithBye = positionedSeeds
    .filter(assignment => seedDrawPositionsWithBye.includes(assignment.drawPosition))
  ...
  const { filteredQuarters } = verifyByeDistribution({ drawSize, byeAssignedDrawPositions });
...
function verifyByeDistribution({ drawSize, byeAssignedDrawPositions}) {
  ...
  const filteredQuarters = quarters.map(quarter => quarter.filter(drawPosition => byeAssignedDrawPositions.includes(drawPosition)));
