...
export function generateEliminationWithQualifying({
  ...
  drawEngine.setStageDrawSize({ stage: QUALIFYING, drawSize: qualifyingDrawSize });
  drawEngine.generateDrawType({ stage: QUALIFYING, drawType: ELIMINATION, qualifyingRound, qualifyingPositions });
  
  const { structures: qualifyingStructures } = drawEngine.getDrawStructures({ stage: QUALIFYING, stageSequence: 1});
  ...
  const { qualifiersCount } = drawEngine.getStructureQualifiersCount({structure: qualifyingStructure});
  ...
  const totalParticipantsCount = qualifyingParticipantsCount + mainParticipantsCount - qualifiersCount;
  const participants = generateRange(0, totalParticipantsCount).map(i => ({participantId: `ko-uuid${i+1}`}));
  const qualifyingParticipants = participants.slice(0, qualifyingParticipantsCount);
  const qualifyingParticipantIds = qualifyingParticipants.map(p=>p.participantId);
  ...
  drawEngine.addDrawEntries({ stage: QUALIFYING, participantIds: qualifyingParticipantIds });
  drawEngine.addDrawEntries({ stage: MAIN, participantIds: mainDrawParticipantIds });

  const alternateParticipants = generateRange(0, alternatesCount).map(i => ({participantId: `alt-uuid${i+1}`}));
  const alternateParticipantIds = alternateParticipants.map(p=>p.participantId);
  drawEngine.addDrawEntries({ stage: QUALIFYING, participantIds: alternateParticipantIds, entryType: ALTERNATE });
  
  const { structures: mainStructures } = drawEngine.getDrawStructures({ stage: MAIN, stageSequence: 1});
  const [mainStructure] = mainStructures;
  const { structureId: mainStructureId } = mainStructure;

  drawEngine.createQualifyingLink({qualifyingStructureId, mainStructureId, qualifyingRound: roundLimit});
  drawEngine.initializeStructureSeedAssignments({ structureId: qualifyingStructureId, seedsCount: qualifyingSeedsCount });
  drawEngine.initializeStructureSeedAssignments({ structureId: mainStructureId, seedsCount: mainSeedsCount });
  ...
  generateRange(1, assignQualifyingSeeds + 1).forEach(seedNumber => {
    ...
    drawEngine.assignSeed({structureId: qualifyingStructureId, seedNumber, seedValue, participantId});
  });
