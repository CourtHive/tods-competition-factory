...
export function generateDrawStructure({
  ...
  drawEngine.generateDrawType({ stage, drawType, qualifyingRound, qualifyingPositions });
  ...
  if (participants && participantsCount !== undefined) {
    ...
    participants = participants || generateRange(0, participantsCount)
  ...
  drawEngine.addDrawEntries({ stage: MAIN, participantIds: alternateParticipantIds, entryType: ALTERNATE });
  ...
  generateRange(1, assignSeeds + 1).forEach(seedNumber => {
    ...
    drawEngine.assignSeed({structureId, seedNumber, seedValue, participantId});
  });
