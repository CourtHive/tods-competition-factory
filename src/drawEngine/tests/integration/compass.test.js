...
it('can generate COMPASS and fill all drawPositions', () => {
  ...
  expectedByeDrawPositions = {
    ...
    'SOUTHEAST': {
      ...
      advancedFiltered: [0],
      pendingAdvancedLength: 1, pendingAdvancedRoundPosition: 1, pendingAdvancedDrawPositions: [2,3],
      unadvancedPending: 3, pendingUnadvancedDrawPositions: [[1,2], [3,4], [undefined, undefined]],
  ...
  expectedByeDrawPositions = {
    ...
    'SOUTHEAST': {
      includeByeMatchUps: true,
      advancedFiltered: [0],
      pendingAdvancedLength: 0, pendingAdvancedRoundPosition: undefined, pendingAdvancedDrawPositions: undefined,
      unadvancedFiltered: 0, unadvancedPending: 1,
    }
  };
  expectedByeRemoval = {
    initialByeMatchUpCount: 15,
    clearExpect: [[0,12], [1,9], [2,5]]
  };
...
function testByeRemoval({stage, expectedByeRemoval}) {
  ...
  const { byeMatchUps } = getDrawMatchUps({drawDefinition, includeByeMatchUps: true});
  ...
  if (expectedByeRemoval.clearExpect) {
    expectedByeRemoval.clearExpect.forEach(([index, length]) => {
      clearDrawPosition({drawDefinition, structureId, drawPosition: byeDrawPositions[index]})
      const { byeMatchUps } = getDrawMatchUps({drawDefinition, includeByeMatchUps: true});
...
function checkCompassByes({stage, expectedCompassByes}) {
  ...
  Object.keys(expectedCompassByes).forEach(direction => {
    ...
    let byeDrawPositions = assignedByes(structure.positionAssignments);
    expect(byeDrawPositions).toMatchObject(expectedCompassByes[direction].byeDrawPositions);
    let byeMatchUpRoundPositions = matchUpsWithBye(structure.matchUps);
    expect(byeMatchUpRoundPositions).toMatchObject(expectedCompassByes[direction].roundPositions);
  })
}

function checkByeAdvancedDrawPositions({stage, advanced, expectedByeDrawPositions}) {
  const { structures } = drawEngine.getDrawStructures({ stage });
  ...
  Object.keys(expectedByeDrawPositions).forEach(direction => {
    ...
    const { pendingMatchUps } = structureMatchUps({structure, includeByeMatchUps});
    ...
    if (advanced) {
      ...
      if (expectedFiltered) {
        ...
        if (filteredMatchUps.length) {
          expect(filteredMatchUps[0].roundPosition).toEqual(expectedFiltered[1]);
          expect(filteredMatchUps[1].roundPosition).toEqual(expectedFiltered[2]);
        }
      }
      ...
      if (pendingMatchUps.length) {
        ...
        if (pendingDrawPositions) expect(pendingMatchUps[0].drawPositions).toMatchObject(pendingDrawPositions);
      }
    } else {
  ...
  function pendingWithOneParticipant(matchUp) {
    return matchUp.roundNumber === 2 && matchUp.drawPositions.filter(f=>f).length === 1;
  }
}
...
function generateCompass({
  ...
  const { structures: [structure] } = drawEngine.getDrawStructures({ stage, stageSequence: 1});
  ...
  checkByeAdvancedDrawPositions({stage, advanced: true, expectedByeDrawPositions});
