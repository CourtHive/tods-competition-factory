...
it('advances paired drawPositions when BYE is assigned first', () => {
  ...
  drawEngine.assignDrawPositionBye({structureId, drawPosition: unassignedPositions[1].drawPosition});
  let { matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 2, roundPosition: 1});
  expect(matchUp.drawPositions).toMatchObject([1,undefined]);
  drawEngine.assignDrawPosition({structureId, drawPosition: unassignedPositions[0].drawPosition, participantId: participantIds[0]});
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 2, roundPosition: 1}));
  expect(matchUp.drawPositions).toMatchObject([1,undefined]);
  ...
  drawEngine.assignDrawPositionBye({structureId, drawPosition: unassignedPositions[6].drawPosition});
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 2, roundPosition: 2}));
  expect(matchUp.drawPositions).toMatchObject([8,undefined]);
  drawEngine.assignDrawPosition({structureId, drawPosition: unassignedPositions[7].drawPosition, participantId: participantIds[1]});
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 2, roundPosition: 2}));
  expect(matchUp.drawPositions).toMatchObject([8,undefined]);
  
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 1, roundPosition: 1}));
  let { matchUpId } = matchUp;
  let { errors } = drawEngine.setMatchUpStatus({matchUpId, matchUpStatus: RETIRED});
  ...
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 1, roundPosition: 2}));
  ({ matchUpId } = matchUp);
  ({ errors } = drawEngine.setMatchUpStatus({matchUpId, matchUpStatus: 'BYE'}));
  expect(errors.length).toBeGreaterThanOrEqual(1);
  
  drawEngine.assignDrawPosition({structureId, drawPosition: unassignedPositions[2].drawPosition, participantId: participantIds[2]});
  drawEngine.assignDrawPosition({structureId, drawPosition: unassignedPositions[3].drawPosition, participantId: participantIds[3]});
  drawEngine.assignDrawPosition({structureId, drawPosition: unassignedPositions[4].drawPosition, participantId: participantIds[4]});
  drawEngine.assignDrawPosition({structureId, drawPosition: unassignedPositions[5].drawPosition, participantId: participantIds[5]});
 
  // add score
  let score, winningSide;
  ({ errors, matchUpId } = completeMatchUp({structureId, roundNumber: 1, roundPosition: 2, winningSide: 1, score: '6-3 6-3' }));
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 1, roundPosition: 2}));
  ...
  result = drawEngine.setMatchUpStatus({matchUpId, winningSide: 2, matchUpStatus: DEFAULTED });
  ...
  result = drawEngine.setMatchUpStatus({matchUpId, matchUpStatus: TO_BE_PLAYED});
  expect(result).toMatchObject(SUCCESS);
...
it('advances paired drawPosition if BYE is assigned second', () => {
  ...
  drawEngine.assignDrawPosition({structureId, drawPosition: unassignedPositions[0].drawPosition, participantId: participantId1});
  let { matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 2, roundPosition: 1});
  ...
  drawEngine.assignDrawPosition({structureId, drawPosition: unassignedPositions[7].drawPosition, participantId: participantId2});
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 2, roundPosition: 2}));
  expect(matchUp.drawPositions).toMatchObject([undefined,undefined]);
  drawEngine.assignDrawPositionBye({structureId, drawPosition: unassignedPositions[6].drawPosition});
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 2, roundPosition: 2}));
...
it('can change a first round matchUp winner and update consolation', () => {
  ...
  ({ errors, matchUpId } = completeMatchUp({structureId, roundNumber: 1, roundPosition: 2, winningSide: 1, score: '6-1 6-2'}));
  expect(errors).toEqual(undefined);
  
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 1, roundPosition: 2, inContext: true}));
  ({ matchUpStatus, Sides, score } = matchUp);
  expect(matchUpStatus).toEqual(COMPLETED);
  expect(score).toEqual('6-1 6-2');

  ({drawDefinition} = getDrawDefinition());
  ({winningParticipantId, losingParticipantId} = getMatchUpWinnerLoserIds({drawDefinition, matchUpId}));
  ...
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 2, roundPosition: 1, inContext: true}));
  ...
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId: consolationStructureId, roundNumber: 1, roundPosition: 1, inContext: true}));
  ({ matchUpStatus, Sides } = matchUp);
  expect(matchUpStatus).toEqual(BYE);
  ...
  ({ matchUp } = findMatchUpByRoundNumberAndPosition({structureId: consolationStructureId, roundNumber: 1, roundPosition: 2}));
  ({ matchUpStatus } = matchUp);
  expect(matchUpStatus).toEqual(TO_BE_PLAYED);
 
  // advance main draw participant to third round
  ({ errors, matchUpId } = completeMatchUp({structureId, roundNumber: 2, roundPosition: 1, winningSide: 1, score: '6-2 6-1'}));
  
  // attempt to complete 2nd position matchUp in first round of consolation draw
  ({ errors, matchUpId } = completeMatchUp({structureId: consolationStructureId, roundNumber: 1, roundPosition: 2, winningSide: 1, score: '6-1 1-6 6-2'}));
  // error because matchUp drawPositions are not assigned to participantIds
  expect(errors.length).toBeGreaterThanOrEqual(1);

  // must direct other participants to consolation draw
  ({ errors, matchUpId } = completeMatchUp({structureId, roundNumber: 1, roundPosition: 3, winningSide: 1, score: '6-1 6-3'}));
  ({ errors, matchUpId } = completeMatchUp({structureId, roundNumber: 1, roundPosition: 4, winningSide: 1, score: '6-1 6-4'}));
  ...
  ({ errors, matchUpId } = completeMatchUp({structureId: consolationStructureId, roundNumber: 2, roundPosition: 1, winningSide: 1, score: '6-2 1-6 6-1'}));
  ({winningParticipantId, losingParticipantId} = getMatchUpWinnerLoserIds({drawDefinition, matchUpId}));
  expect(winningParticipantId).toEqual(consolationParticipantId);

  // Now attempt to change a 1st round matchUpStatus to BYE
  ({ matchUp: { matchUpId } } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 1, roundPosition: 2, inContext: true}));
  ({ errors } = drawEngine.setMatchUpStatus({matchUpId, matchUpStatus: BYE, score: '6-1'}));
  expect(errors.length).toBeGreaterThanOrEqual(1);
 
  // Now attempt to change a 1st round matchUpStatus to TO_BE_PLAYED
  ({ matchUp: { matchUpId } } = findMatchUpByRoundNumberAndPosition({structureId, roundNumber: 1, roundPosition: 2, inContext: true}));
  ({ errors } = drawEngine.setMatchUpStatus({matchUpId, matchUpStatus: TO_BE_PLAYED}));
  expect(errors.length).toBeGreaterThanOrEqual(1);
 
  // Now attempt to change a 1st round matchUpStatus, but not winner...
  result = drawEngine.setMatchUpStatus({matchUpId, matchUpStatus: RETIRED, score: '6-1'});
  expect(result).toMatchObject(SUCCESS);
  
  // Now attempt to change a 1st round matchUpStatus, but not winner...
  result = drawEngine.setMatchUpStatus({matchUpId, matchUpStatus: RETIRED, winningSide: 2, score: '6-1'});
  expect(result).toMatchObject(SUCCESS);
  
  // Now attempt to change a 1st round matchUpStatus to nonDirecting outcome, same winningSide...
  result = drawEngine.setMatchUpStatus({matchUpId, matchUpStatus: SUSPENDED, winningSide: 2, score: '6-1'});
  expect(errors.length).toBeGreaterThanOrEqual(1);
  
  // Now attempt a change which has no available actions...
  result = drawEngine.setMatchUpStatus({matchUpId, winningSide: 2, score: '6-1'});
  expect(errors.length).toBeGreaterThanOrEqual(1);
  
  // Now attempt to change a 1st round matchUp outcome, including winner...
  ({ errors } = drawEngine.setMatchUpStatus({matchUpId, winningSide: 1, matchUpStatus: COMPLETED, score: '6-0 6-0'}));
  expect(errors.length).toBeGreaterThanOrEqual(1);
