...
it('can define seedAssignments', () => {
  ...
  const { structures: stageStructures } = getDrawStructures({drawDefinition, stage, stageSequence: 1});
  const { structureId } = stageStructures[0];

  // generates error if seedsCount is greater than drawSize
  let result = drawEngine.initializeStructureSeedAssignments({structureId, seedsCount});
  expect(result).toHaveProperty(ERROR);
  
  seedsCount = 4;
  result = drawEngine.initializeStructureSeedAssignments({structureId, seedsCount});
  expect(result).toMatchObject(SUCCESS);
 
  // returns drawDefinition by reference
  const { drawDefinition: drawDefinitionAfterAssignments } = getDrawDefinition();
  const { seedAssignments } = getStructureSeedAssignments({ drawDefinition: drawDefinitionAfterAssignments, structureId });
  expect(seedAssignments.length).toEqual(seedsCount);
});

it('generates valild seedBlocks given different policies', () => {
  const ITF_32_expectedBlocks = [
    ...
    { seedNumbers: [9,10,11,12,13,14,15,16], drawPositions: [4,5,12,13,20,21,28,29] },
  ];
  checkSeedBlocks({drawSize: 32, policy: ITF_SEEDING, expectedBlocks: ITF_32_expectedBlocks});
 
  const ITF_64_expectedBlocks = [
    { seedNumbers: [1], drawPositions: [1] },
    { seedNumbers: [2], drawPositions: [64] },
    { seedNumbers: [3,4], drawPositions: [17,48] },
    { seedNumbers: [5,6,7,8], drawPositions: [16,32,33,49] },
    { seedNumbers: [9,10,11,12,13,14,15,16], drawPositions: [8,9,24,25,40,41,56,57] },
  ];
  checkSeedBlocks({drawSize: 64, policy: ITF_SEEDING, expectedBlocks: ITF_64_expectedBlocks});
  
  const USTA_16_expectedBlocks = [
    { seedNumbers: [1], drawPositions: [1] },
    { seedNumbers: [2], drawPositions: [16] },
    { seedNumbers: [3,4], drawPositions: [5,12] },
    { seedNumbers: [5,6,7,8], drawPositions: [3,7,10,14] },
  ];
  checkSeedBlocks({drawSize: 16, policy: USTA_SEEDING, expectedBlocks: USTA_16_expectedBlocks});
 
  const USTA_32_expectedBlocks = [
    { seedNumbers: [1], drawPositions: [1] },
    { seedNumbers: [2], drawPositions: [32] },
    { seedNumbers: [3,4], drawPositions: [9,24] },
    { seedNumbers: [5,6,7,8], drawPositions: [5,13,20,28] },
    { seedNumbers: [9,10,11,12,13,14,15,16], drawPositions: [3,7,11,15,18,22,26,30] },
  ];
  checkSeedBlocks({drawSize: 32, policy: USTA_SEEDING, expectedBlocks: USTA_32_expectedBlocks});
 
  const USTA_64_expectedBlocks = [
    { seedNumbers: [1], drawPositions: [1] },
    { seedNumbers: [2], drawPositions: [64] },
    { seedNumbers: [3,4], drawPositions: [17,48] },
    { seedNumbers: [5,6,7,8], drawPositions: [9,25,40,56] },
    { seedNumbers: [9,10,11,12,13,14,15,16], drawPositions: [5,13,21,29,36,44,52,60] },
  ];
  checkSeedBlocks({drawSize: 64, policy: USTA_SEEDING, expectedBlocks: USTA_64_expectedBlocks});
 
  const USTA_128_expectedBlocks = [
    { seedNumbers: [1], drawPositions: [1] },
  ...
  checkSeedBlocks({drawSize: 128, policy: USTA_SEEDING, expectedBlocks: USTA_128_expectedBlocks});
 
  const USTA_256_expectedBlocks = [
    { seedNumbers: [1], drawPositions: [1] },
  ...
  checkSeedBlocks({drawSize: 256, policy: USTA_SEEDING, expectedBlocks: USTA_256_expectedBlocks});
});
...
it('can assign seedNumbers and drawPositions to seeded participants', () => {
  ...
  result = drawEngine.assignSeed({structureId, seedNumber: 17, participantId});
  expect(result).toHaveProperty(ERROR);
  
  drawDefinition = drawEngine.getState();
  let { unplacedSeedNumbers, unfilledPositions } = drawEngine.getNextSeedBlock({structureId});
  let seedNumber = unplacedSeedNumbers.pop();
  ...
  result = drawEngine.assignDrawPosition({structureId, drawPosition: 2, participantId});
  expect(result).toHaveProperty(ERROR);
  ...
  result = drawEngine.assignDrawPosition({structureId, drawPosition: 2, participantId: participantId2});
  expect(result).toMatchObject(SUCCESS);
  
  // attempt to assign a seedNumber to a participant already in a non-seed drawPosition
  result = drawEngine.assignSeed({structureId, seedNumber: 2, participantId: participantId2});
  expect(result).toHaveProperty(ERROR);

  // assign an unseeded participant to a drawPosition which *is* a seed position
  result = drawEngine.assignDrawPosition({structureId, drawPosition: 64, participantId: participantId2});
  ...
  result = drawEngine.clearDrawPosition({structureId, participantId: participantId2});
  expect(result).toMatchObject(SUCCESS);
  ...
  ({ unplacedSeedNumbers, unfilledPositions } = drawEngine.getNextSeedBlock({structureId}));
  expect(unplacedSeedNumbers).toMatchObject([3,4]);
  expect(unfilledPositions).toMatchObject([17,48]);
  
  // nominate participant as 4th seed
  result = drawEngine.assignSeed({structureId, seedNumber: 4, participantId: participantId3});
  expect(result).toMatchObject(SUCCESS);

  drawPosition = unfilledPositions.pop();
  expect(drawPosition).toEqual(48);
  result = drawEngine.assignDrawPosition({structureId, drawPosition, participantId: participantId3});
  expect(result).toMatchObject(SUCCESS);
...
function checkSeedBlocks({drawSize, policy, expectedBlocks}) {
  const { structure: { structureId }} = mainDrawWithEntries({drawSize});
  ...
  validSeedBlocks.forEach((seedBlock, i) => {
    expect(seedBlock.seedNumbers.sort(numericSort)).toMatchObject(expectedBlocks[i].seedNumbers.sort(numericSort));
    expect(seedBlock.drawPositions.sort(numericSort)).toMatchObject(expectedBlocks[i].drawPositions.sort(numericSort));
  })
}
