...
export function treeMatchUps({ drawSize, qualifyingPositions, roundLimit, qualifyingRound, finishingPositionOffset }) {
    if (isNaN(drawSize) || !powerOf2(drawSize) || drawSize < 2) {
        return { matchUps: [], roundsCount: 0 };
    }

    let nodes = generateRange(1, drawSize + 1).map(drawPosition => ({ drawPosition }));
    ...
    while (roundNodes.length > 1) {
        if (qualifyingPositions && roundNodes.length === qualifyingPositions) roundLimit = roundNumber - 1;
        ({roundNodes, matchUps} = buildRound({roundNumber, nodes: roundNodes, matchUps}));
        roundNumber++;
    }
    ...
    if (roundLimit) { matchUps = matchUps.filter(matchUp => matchUp.roundNumber <= roundLimit); }

    return { matchUps, roundsCount, roundLimit };
}

function addFinishingRounds({matchUps, roundsCount, roundLimit, finishingPositionOffset=0}) {
    ...
    let roundMatchCountArray = Object.values(roundMatchCounts);

    // returns a range for array of possible finishing drawPositions
    const finishingRange = drawPositions => [Math.min(...drawPositions), Math.max(...drawPositions)];
    ...
    matchUps.forEach(matchUp => {
        matchUp.finishingRound = roundsCount + 1 - matchUp.roundNumber - finishingRoundOffset;

        let rangeOffset = 1 + finishingPositionOffset;
        let currentMatchUps = roundMatchCounts[matchUp.roundNumber];
        let upcomingMatchUps = roundMatchCountArray
            .slice(matchUp.roundNumber - 1)
            .reduce((a, b) => a + b, 0);
       
        let finalPosition = 1;
        let finishingPositionRange = generateRange(rangeOffset, upcomingMatchUps + rangeOffset + finalPosition)
        let slicer = upcomingMatchUps + finalPosition - currentMatchUps;
        let loser = finishingRange(finishingPositionRange.slice(slicer));
        let winner = finishingRange(finishingPositionRange.slice(0, slicer));
...
function buildRound({roundNumber, nodes, matchUps}) {
    ...
    let roundNodes = [];
    let roundPosition = 1;
    let matchRoundNumber = roundNumber - 1;
    const roundMatchUpsCount = nodes.length;
    while (index < roundMatchUpsCount) {
        let child1 = nodes[index];
        let child2 = nodes[index + 1];
...
export function feedDrawSize({ opponentCount }) {
    ...
    const positionsBase = baseRanges.reduce((p, c) => {
        return opponentCount >= c.positionsBase && opponentCount <= c.maxPositions ? c : p;
    }, undefined)
...
export function feedInMatchUps({
    ...
    let firstRoundDrawPositions = generateRange(0, baseDrawSize).map((x, i) => ({ drawPosition: i + 1 + positionsFed }));
    ...
    for (const baseDrawRound of baseDrawRounds) {
        ...
        if (feedRoundsProfile.includes(baseDrawRound)) {
            ...
            const isLinkFedRound = linkFedFinishingRoundNumbers.includes(finishingRoundNumber) ||
                linkFedRoundNumbers.includes(roundNumber);

            for (const roundIteration of iterationRange) {
                let iterationDrawPosition = (!isLinkFedRound && drawPosition) || undefined;
    ...
    let draw = roundNodes && roundNodes.length ? roundNodes[0] : roundNodes;
...
function buildFeedRound({nodes, feedMode, drawPosition, fed, matchUps, roundNumber}) {
    ...
    const drawPositionGroup = generateRange(0, feedRoundMatchUpsCount)
        .map(value => initialGroupDrawPosition ? initialGroupDrawPosition + value : undefined);
        
    let roundNodes = [];
    for (let nodeIndex=0; nodeIndex < feedRoundMatchUpsCount; nodeIndex++) {
        let feedDrawPosition = getPosition();
        ...
        let matchUp = { children: [ position, feed_arm] };
