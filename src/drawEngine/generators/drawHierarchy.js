...
import { generateRange, makeDeepCopy, unique, UUID } from 'competitionFactory/utilities';
...
export function buildDrawHierarchy({matchUps}) {
    ...
    const allDrawPositions = unique(matchUps.map(matchUp => matchUp.drawPositions).flat()).sort(drawPositionSort);
    ...
    const additionalRoundMatchUps = generateRange(1, additionalRounds + 1)
        .map(finishingRound => {
    ...
    if (additionalRounds) { matchUps = matchUps.concat(...additionalRoundMatchUps.flat()); }
    ...
    const secondRoundEntriySides = secondRoundMatchUps
        .filter(matchUp => matchUp.drawPositions.reduce((p, c) => secondRoundEntries.includes(c) || p, undefined))
        .map(matchUp => {
           const targetDrawPosition = matchUp.drawPositions.reduce((p, c) => secondRoundEntries.includes(c) ? c : p, undefined);
    ...
    if (missingDrawPositions.length && secondRoundEntries.length === missingDrawPositions.length) {
        ...
        const finishingPositionRange = makeDeepCopy(firstRoundMatchUps[0].finishingPositionRange);
    ...
    roundsIterator.forEach(roundNumber => {
        ...
        const previousRoundMatchUps = filterRoundMatchUps({matchUps, roundNumber: roundNumber - 1});
        const previousRoundWinnerIds = previousRoundMatchUps.map(getWinningParticipantId).filter(f=>f);
        ...
        if (roundNumber === 1) {
            roundMatchUps.forEach((matchUp, i) => {
                ...
                const children = [
                    { ...matchUp.Sides[0], drawPosition: drawPositions[0], matchUpId, structureId },
                    { ...matchUp.Sides[1], drawPosition: drawPositions[1], matchUpId, structureId }
                ];
            ...
            roundMatchUps.forEach((matchUp, i) => {
                ...
                const fedDrawPosition = drawPositions
                    ...
                    .reduce((fed, position) => { // fed position is not included in first round
                ...
                const children = [
                    { ...fedSide, drawPosition: fedDrawPosition, feedRoundNumber, Sides: matchUp.Sides, matchUpId, structureId },
                    previousRound[i]
                ];
...
function getWinningParticipantId(matchUp) {
    ...
    const participantId = matchUp.Sides.reduce((p, c) => c.sideNumber === matchUp.winningSide ? c.participantId : p, undefined);
