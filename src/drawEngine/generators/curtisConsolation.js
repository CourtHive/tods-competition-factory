...
export function generateCurtisConsolation({
  ...
  const {
    matchUps,
    roundsCount: mainDrawRoundsCount
  } = treeMatchUps({ drawSize, finishingPositionOffset });
  ...
  const consolationItems = feedRoundOffsets.map((roundOffset, index) => {
    ...
    } = consolationFeedStructure({ drawSize, index, roundOffset, stageSequence });
  ...
  let links = consolationItems.map(item => item.links);
  ...
  if ((drawSize >= 4 && drawSize <= 16) || drawSize > 32) {
    ...
    } = treeMatchUps({ drawSize: 2, finishingPositionOffset: 2 });
...
function consolationFeedStructure({ drawSize, index, roundOffset=0, stageSequence=1 }) {
