import { START_TIME, STOP_TIME, RESUME_TIME, END_TIME } from 'competitionFactory/constants/timeItemConstants';
...
export function matchUpDuration({matchUp}) {
  ...
  const relevantTimeItems = matchUp.timeItems
    .filter(timeItem => [START_TIME, STOP_TIME, RESUME_TIME, END_TIME].includes(timeItem.itemSubject))
  ...
  const elapsed = relevantTimeItems.reduce((elapsed, timeItem) => {
    ...
    switch(timeItem.itemSubject) {
      case START_TIME: 
        ...
        break;
      case END_TIME:
        if (elapsed.lastValue && [START_TIME, RESUME_TIME].includes(elapsed.lastSubject)) {
          const interval = new Date(timeItem.itemValue) - new Date(elapsed.lastValue);
    ...
    return { milliseconds, lastSubject: timeItem.itemSubject, lastValue: timeItem.itemValue };
  ...
  return { milliseconds: elapsed.milliseconds, time: msToTime(elapsed.milliseconds), relevantTimeItems };
}
