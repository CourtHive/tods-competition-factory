import { countValues, numericSort } from 'competitionFactory/utilities';
...
import { CONTAINER } from 'competitionFactory/constants/drawDefinitionConstants';

// TODO: write unit test for this method
export function getDrawPositions({structure}) {
  if (structure && structure.structures) {
    return [].concat(...structure.structures.map(structure => getDrawPositions({structure})));
  } else if (structure) {
...
export function getPositionAssignments({ structure, drawDefinition, structureId }) {
  ...
  if (structure.structures) {
    positionAssignments = [].concat(...structure.structures.map(structure => {
...
// TESTED: positionsAssignment.test.js
export function structureAssignedDrawPositions({ structure, drawDefinition, structureId }) {
  const { positionAssignments } = getPositionAssignments({ structure, drawDefinition, structureId });
...
export function structureActiveDrawPositions({ drawDefinition, structureId }) {
  ...
  const { positionAssignments } = getPositionAssignments({ structure, drawDefinition });
  const byeDrawPositions = positionAssignments
    .filter(assignment => assignment.bye)
    .map(assignment => assignment.drawPosition);
  const byePairedPositions = byeDrawPositions.map(getByePairedPosition).flat(Infinity);

  if (structure.structureType === CONTAINER) {
    const relevantMatchUps = matchUps.filter(matchUp => matchUp.score || matchUp.winningSide);
    let activeDrawPositions = [].concat(...relevantMatchUps.map(matchUp => matchUp.drawPositions || []))
      .filter(f=>f).sort(numericSort);
    
    return { activeDrawPositions, advancedDrawPositions: [], pairedDrawPositions: [], byeDrawPositions };
  } else {
    let drawPositions = [].concat(...matchUps.map(matchUp => matchUp.drawPositions || []))
      .filter(f=>f).sort(numericSort);

    // now remove ONE INSTANCE of byePairedPositions from drawPositions
    const instancesToRemove = [].concat(...byePairedPositions, ...byeDrawPositions);
    ...
    const advancedDrawPositions = Object.keys(positionCounts).reduce((active, key) => {
    ...
    const pairedDrawPositions = [].concat(...advancedDrawPositions.map(getPairedDrawPositions));
    const activeDrawPositions = [].concat(...advancedDrawPositions, pairedDrawPositions).filter(f=>f);

    return { activeDrawPositions, advancedDrawPositions, pairedDrawPositions, byeDrawPositions };
  }
  
  function getPairedDrawPositions(drawPosition) {
    return matchUps.reduce((drawPositions, currentMatchup) => {
      return currentMatchup.drawPositions.includes(drawPosition) ? drawPositions.concat(...currentMatchup.drawPositions) : drawPositions;
