...
export function stageExists({stage, drawDefinition}) {
  return drawDefinition && stage && Object.keys(drawDefinition.entryProfile).includes(stage);
}
export function stageStructures({stage, drawDefinition, stageSequence}) {
  return drawDefinition && stage && drawDefinition.structures && drawDefinition.structures.filter(structure => {
...
export function getStageDrawPositionsAvailable({stage, drawDefinition}) {
  ...
  const qualifyingPositions = getStageQualifiersCount({stage, drawDefinition});
  return drawSize && drawSize - qualifyingPositions;
}
export function stageAlternates({stage, drawDefinition}) { return drawDefinition.entryProfile[stage].alternates; }
export function getStageWildcardsCount({stage, drawDefinition}) { return drawDefinition.entryProfile[stage].wildcardsCount || 0; }
export function getStageEntryTypeCount({stage, drawDefinition, entryType}) {
  return drawDefinition.entries.reduce((p, c) => c.entryStage === stage && c.entryType === entryType ? p+1 : p, 0);
}
...
export function stageEntries({stage, stageSequence, drawDefinition, entryTypes}) {
...
export function getStageDirectEntriesCount({ stage, drawDefinition }) {
  return getStageEntryTypeCount({ stage, drawDefinition, entryType: DIRECT_ACCEPTANCE });
}
...
export function stageAlternateEntries({ stage, drawDefinition}) {
  return getStageEntryTypeCount({ stage, drawDefinition, entryType: ALTERNATE });
}
export function stageSpace({stage, drawDefinition, entryType=DIRECT_ACCEPTANCE}) {
  ...
  const stageDrawPositionsAvailable = getStageDrawPositionsAvailable({stage, drawDefinition});
  const wildcardPositions = getStageWildcardsCount({stage, drawDefinition});
  const wildcardEntriesCount = getStageEntryTypeCount({stage, drawDefinition, entryType: WILDCARD});
  const directEntriesCount = getStageEntryTypeCount({stage, drawDefinition, entryType: DIRECT_ACCEPTANCE});
