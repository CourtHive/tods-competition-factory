...
*/
export function nextRoundMatchUp({structure, tournamentParticipants, matchUp}) {
  const { roundNumber, roundPosition } = matchUp;
  const { matchUps } = getAllStructureMatchUps({structure, tournamentParticipants, inContext: true});
  const currentRoundMatchUps = matchUps.filter(matchUp => matchUp.roundNumber === roundNumber);
  const nextRoundMatchUps = matchUps.filter(matchUp => matchUp.roundNumber === roundNumber + 1);
  ...
  if (nextRoundMatchUps.length) {
    ...
    if (nextRoundMatchUps.length === currentRoundMatchUps.length) {
      ...
      nextMatchUp = nextRoundMatchUps.reduce((matchUp, current) => {
        return current.roundPosition === Math.ceil(roundPosition/2) ? current : matchUp;
      }, undefined);
    }
