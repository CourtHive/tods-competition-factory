export function getMatchUpLinks({drawDefinition, matchUp, structureId}) {
  ...
  if (!structureId) { ({ structureId } = matchUp); }

  const { links } = getStructureLinks({ drawDefinition, structureId });
  const source = links.source.reduce((source, link) => {
    return link.source.roundNumber === matchUp.roundNumber ? source.concat(link) : source;
  }, []);
  const target = links.target.reduce((target, link) => {
    return link.target.roundNumber === matchUp.roundNumber ? target.concat(link) : target;
  }, []);
...
export function getStructureLinks({drawDefinition, structureId}) {
  ...
  const structureLinks = links.reduce((structureLinks, link) => {
    if (link.source.structureId === structureId) structureLinks.source = structureLinks.source.concat(link);
    if (link.target.structureId === structureId) structureLinks.target = structureLinks.target.concat(link);
