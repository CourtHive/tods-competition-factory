...
export function getDrawStructures({stage, stageSequence, drawDefinition}={}) {
  ...
  function isStage(structure) { return structure.stage === stage; }
  function isStageSequence(structure) { return !stageSequence || structure.stageSequence === stageSequence; }
}
...
export function drawStructures({stage, stageSequence, drawDefinition}) {
  const error = !drawDefinition ? 'Missing drawDefinition' : 
    !drawDefinition.structures ? 'Missing structures' : undefined;
  const structures = (drawDefinition && drawDefinition.structures.filter(isStage).filter(isStageSequence)) || [];
  return { structures, error };

  function isStage(structure) { return !stage || structure.stage === stage; }
  function isStageSequence(structure) { return !stageSequence || structure.stageSequence === stageSequence; }
}

export function getStructureSeedAssignments({ drawDefinition, structure, structureId}) {
...
export function getStructureQualifiersCount({ drawDefinition, structure, structureId}) {
