...
import { allTournamentMatchUps } from 'competitionFactory/tournamentEngine/getters/matchUpsGetter'

import { SUCCESS } from "competitionFactory/constants/resultConstants";
import { COURT, ASSIGNMENT } from 'competitionFactory/constants/timeItemConstants';
...
export function deleteVenue({tournamentRecord, drawEngine, venueId}) {
  ...
  const { matchUps: matchesToUnschedule } = allTournamentMatchUps({tournamentRecord, drawEngine, contextFilters});
  
  matchesToUnschedule.forEach(matchUp => {
    if (matchUp.timeItems) {
      const hasCourtAssignment = matchUp.timeItems.reduce((hasAssignment, candidate) => {
  ...
  tournamentRecord.venues = tournamentRecord.venues.filter(venue => venue.venueId !== venueId);
