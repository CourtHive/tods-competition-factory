...
export function setParticipantScaleItem({ tournamentRecord, participantId, scaleItem }) {
  ...
  const requiredAttributes = ['scaleType', 'eventType', 'scaleName', 'scaleValue'];
  ...
  if (participantId && validScaleItem && Array.isArray(tournamentRecord.participants)) {
...
export function setParticipantScaleItems({ tournamentRecord, scaleItemsWithParticipantIDsArray }) {
  let modificationApplied
  let participantScaleItemsMap = {};
  ...
  if (tournamentRecord && tournamentRecord.participants) {
    ...
    return modificationApplied ? SUCCESS : { error: 'No Modifications Applied' };
  }
}
