...
export function countValues(values) { return groupValues(instanceCount(values)); }
...
export function groupValues(obj) {
  return Object.keys(obj)
    .reduce((p, c) => { const value = obj[c]; if (p[value]) {p[value].push(c)} else {p[value]=[c]}; return p; }, {});
}

function onlyUnique(value, index, self) { return self.indexOf(value) === index; }
export function uniqueValues(arr) { return arr.filter(onlyUnique); }
...
export function arrayOfLength(count) { return [...Array(count)].map((_, i) => i); }
export function generateRange(start, end) { return Array.from({length: (end - start)}, (v, k) => k + start); }
export function indices(val, arr) { return arr.reduce((a, e, i) => { if (e === val) a.push(i); return a; }, []); }
export function arrayIndices(val, arr) { return arr.reduce((a, e, i) => { if (e === val) a.push(i); return a; }, []); }
export function intersection(a, b) { return a.filter(n => b.indexOf(n) !== -1).filter((e, i, c) => c.indexOf(e) === i); }
export function randomPop(array) {
    return Array.isArray(array) && array.length ? array.splice(Math.floor(Math.random()*array.length), 1)[0] : undefined;
}
export function occurrences(val, arr) { return arr.reduce((r,val) => { r[val] = 1+r[val] || 1; return r; },{})[val] || 0; }
