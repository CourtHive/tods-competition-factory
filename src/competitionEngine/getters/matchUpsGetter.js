...
export function allCompetitionMatchUps({tournamentRecords, tournamentEngine, matchUpFilters, contextFilters}) {
  if (!tournamentRecords) return { error: 'Missing tournamentRecords' };
  
  const tournamentIds = Object.keys(tournamentRecords);
  const competitionMatchUps = tournamentIds.map(tournamentId => {
...
export function competitionScheduleMatchUps(props) {
  ...
  const { completedMatchUps, upcomingMatchUps, pendingMatchUps } = competitionMatchUps(props);
  const dateMatchUps = [...(upcomingMatchUps || []), ...(pendingMatchUps || [])]
...
export function competitionMatchUps({tournamentRecords, tournamentEngine, matchUpFilters, contextFilters}) {
  if (!tournamentRecords) return { error: 'Missing tournamentRecords' };
  ...
  const matchUpGroupings = tournamentsMatchUps.reduce((groupings, tournamentMatchUps) => {
    let keys = Object.keys(tournamentMatchUps);
    keys.forEach(key => {
