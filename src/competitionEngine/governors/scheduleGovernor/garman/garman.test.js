...
function courtGenerator({count=10, startTime='8:00', endTime='20:30'}={}) {
    return generateRange(0,count).map(i => ({
        dateAvailability: [ { date, startTime, endTime, } ]
    }))
}

it('generates dummy courts', () => {
  let courts = courtGenerator({count: 10, startTime:'8:00', endTime:'20:30'});
...
it('replcates revised garman spreadsheet funcationality', () => {
  let courts = courtGenerator({count:10, startTime:'8:00', endTime:'20:30'});
  let { timingProfile } = garman.matchUpTiming({startTime:'8:00', endTime:'19:00', date, periodLength:30, averageMatchTime:90, courts });
  ...
  ({timingProfile} = garman.matchUpTiming({startTime:'8:00', endTime:'19:00', date, periodLength:30, averageMatchTime:80, courts }));
  ...
  ({timingProfile} = garman.matchUpTiming({startTime:'8:00', endTime:'19:00', date, periodLength:30, averageMatchTime:75, courts }));
...
it('correctly determines courts available at periodStart', () => {
    ...
    let availability = garman.courtsAvailableAtPeriodStart({ courts, date, periodStart: '8:00', averageMatchTime: 90 });
    expect(availability.count).toEqual(0);
    availability = garman.courtsAvailableAtPeriodStart({ courts, date, periodStart: '9:00', averageMatchTime: 90 });
    expect(availability.count).toEqual(2);
    availability = garman.courtsAvailableAtPeriodStart({ courts, date, periodStart: '11:00', averageMatchTime: 90 });
    expect(availability.count).toEqual(4);
    availability = garman.courtsAvailableAtPeriodStart({ courts, date, periodStart: '15:30', averageMatchTime: 90 });
    expect(availability.count).toEqual(4);
    availability = garman.courtsAvailableAtPeriodStart({ courts, date, periodStart: '15:31', averageMatchTime: 90 });
    expect(availability.count).toEqual(2);
    availability = garman.courtsAvailableAtPeriodStart({ courts, date, periodStart: '16:30', averageMatchTime: 90 });
    expect(availability.count).toEqual(2);
    availability = garman.courtsAvailableAtPeriodStart({ courts, date, periodStart: '16:31', averageMatchTime: 90 });
    expect(availability.count).toEqual(0);
});
...
it('generates expected output for two locations with differing court availability', () => {
    ...
    let { timingProfile } = garman.matchUpTiming({startTime:'8:00', endTime:'18:00', date, periodLength:30, averageMatchTime:90, courts });
...
it('can generate timeSlots from dateAvailability', () => {
    ...
    const timeSlots = garman.generateTimeSlots({courtDate, includeBookingTypes: ['match']});
