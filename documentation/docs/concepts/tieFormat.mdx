...
When context is added to `matchUps` the engines of the Competition Factory utilize the `lineUps` and the `tieFormat` to include all relevant information, such as `sides` which include `participants`.
