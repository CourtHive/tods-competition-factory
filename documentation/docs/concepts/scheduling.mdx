...
Schedule Conflicts are reported for participants that are returned from `tournamentEngine.getTournamentParticipants` or `competitionEngine.getCompetitionParticipants`.

The attribute `scheduleConflicts` is an array of `matchUpIds` that have been scheduled at times that conflict with other `matchUps` based on either the **scheduling policy**
