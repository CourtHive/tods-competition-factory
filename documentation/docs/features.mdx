...
See [Generating Tournaments](engines/mocks-engine-examples) for live/interactive examples of tournament generation.
...
function DrawDefinitionDemo(props) {
  ...
  // add an event
  const { event } = tournamentEngine.addEvent({ eventName: 'Demo' });

  // enter all participants into the event
