...
it may also instruct [generateDrawDefinition](/docs/governors/generation-governor#generatedrawdefinition) to disallow the placement of seed numbers in positions which are not defined as seeded positions,
as well as indicate that duplicate seed numbers may be used.

<JsonViewer data={PolicySeeding} root={'seeding'} colorScheme={'summerfruit'} invertTheme={true} expandToLevel={1} />
